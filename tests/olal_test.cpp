...
TEST(OlalTest, SimpleSpatialDistributionTest) {
    ...
    optimize_trip(c1->trip, spatial_uniform);
...
TEST(OlalTest, GroupSpatialDistributionTest) {
    // Initialize network
    Network* network = new_network();
    // Nodes
    Node* n1 = new_node(network, "Albisrieden", 0.0, 0.0);
    ...
    // Add reservations
    Reservation* r1 = new_reservation(c1, 3, NULL);
    Reservation* r2 = new_reservation(c2, 1, NULL);
    Reservation* r3 = new_reservation(c3, 1, NULL);
    Reservation* r4 = new_reservation(c4, 1, NULL);
    Reservation* r5 = new_reservation(c5, 1, NULL);
    ...
    EXPECT_EQ(r1->seat_arr[0]->seat_id, train->seats[3]->seat_id);
    EXPECT_EQ(r1->seat_arr[1]->seat_id, train->seats[4]->seat_id);
    EXPECT_EQ(r1->seat_arr[2]->seat_id, train->seats[2]->seat_id);
    EXPECT_EQ(r2->seat_arr[0]->seat_id, train->seats[1]->seat_id);
    EXPECT_EQ(r3->seat_arr[0]->seat_id, train->seats[5]->seat_id);
    EXPECT_EQ(r4->seat_arr[0]->seat_id, train->seats[0]->seat_id);
    EXPECT_EQ(r5->seat_arr[0]->seat_id, train->seats[6]->seat_id);
